...
if (window.sessionStorage.getItem('firebase:session::bellroyinstagram')) {
  $('#contentWrapper').show()
  $('#loggedIn').html('Logged in as ' + JSON.parse(window.sessionStorage.getItem('firebase:session::bellroyinstagram')).password.email + '<div><button id="logOut">Log out</button></div>')
  $('#loggedIn').show()
  $('#login').hide()
}
...
$('#bellroybutton').click(function () {
  ...
  bellroyFeed = new window.Instafeed({
    get: 'user',
    clientId: myClientId,
...
$('#searchbutton').click(function () {
  ...
  var target = 'tagged'
  searchFeed = new window.Instafeed({
    get: target,
    tagName: input,
    clientId: myClientId,
    resolution: 'standard_resolution',
...
$('#submitSelected').click(function () {
  if ($('#nameInput').val() !== '') {
    ...
    var slackPost = new window.XMLHttpRequest()
    ...
    slackPost.onreadystatechange = function () {
      if (slackPost.status === 200 && !alerted) {
        ...
        window.alert('Sam successfully notified')
      }
    }
  } else {
    window.alert('Please enter the slug you wish to associate this feed with')
